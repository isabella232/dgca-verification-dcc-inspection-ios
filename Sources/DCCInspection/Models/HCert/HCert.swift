...
public class HCert: CertificationProtocol, Codable {
    ...
    public var cryptographicallyValid: Bool {
        if !DCCInspection.config.checkSignatures {
            return true
        }
        guard let delegate = DCCInspection.publicKeyEncoder else { return false }
  ...
  private func getCertificateCreationDate() -> String {
    let testStatementObject = get(.testStatements).rawValue
    if let testStatement = testStatementObject.first {
